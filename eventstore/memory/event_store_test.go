		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.haveEvents != nil {
				for _, eventSet := range tt.haveEvents {
					if _, err = store.AppendStream(t.Context(), eventSet.streamID, eventSet.events, eventstore.AppendStreamOptions{}); err != nil {
			...
			_, gotErr := store.AppendStream(t.Context(), tt.haveStreamID, tt.haveAppendEvents, tt.haveAppendOpts)
			if tt.wantErr != nil {
				if gotErr == nil || gotErr.Error() != tt.wantErr.Error() {
			...
			iter, err := store.ReadStream(t.Context(), tt.haveStreamID, eventstore.ReadStreamOptions{})
			if err != nil {
...
func TestEventStore_ReadStream(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			iter, err := store.ReadStream(t.Context(), tt.haveStreamID, tt.haveReadStreamOpts)
			if tt.wantErr != nil {
			...
			for {
				gotEvent, err := iter.Next(t.Context())
				if errors.Is(err, eventstore.ErrEndOfEventStream) {
					break
				} else if err != nil {
					t.Fatalf("Next() error: %v", err)
...
func TestEventStore_Metadata(t *testing.T) {
	...
	if _, err := store.AppendStream(t.Context(), streamID, []*eventstore.WritableEvent{
...
func TestEventStore_GlobalPosition(t *testing.T) {
	...
	if _, err := store.AppendStream(t.Context(), streamID1, []*eventstore.WritableEvent{
		{Type: "eventtype1", Data: []byte("s1e1")},
		{Type: "eventtype1", Data: []byte("s1e2")},
	}, eventstore.AppendStreamOptions{}); err != nil {
		t.Fatalf("AppendStream() error: %v", err)
	}
	if _, err := store.AppendStream(t.Context(), streamID2, []*eventstore.WritableEvent{
	...
	iter1, err := store.ReadStream(t.Context(), streamID1, eventstore.ReadStreamOptions{})
	if err != nil {
		t.Fatalf("ReadStream() error: %v", err)
	}
	var prevPos int64
	for {
		event, err := iter1.Next(t.Context())
		if errors.Is(err, eventstore.ErrEndOfEventStream) {
			break
		}
		if err != nil {
	...
	iter2, err := store.ReadStream(t.Context(), streamID2, eventstore.ReadStreamOptions{})
	if err != nil {
		t.Fatalf("ReadStream() error: %v", err)
	}
	event, err := iter2.Next(t.Context())
	if err != nil {
		t.Fatalf("Next() error: %v", err)
	}
	if event.GlobalPosition == nil {
...
func TestEventStore_ErrorTypes(t *testing.T) {
	...
	t.Run("StreamVersionMismatchError satisfies errors.Is", func(t *testing.T) {
		...
		_, gotErr := store.AppendStream(t.Context(), streamID, []*eventstore.WritableEvent{
