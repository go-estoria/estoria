...
func TestNew_ReservedStreamTypePrefix(t *testing.T) {
	...
	t.Run("allows the library's own projection stream type", func(t *testing.T) {
		t.Parallel()

		if _, err := aggregatestore.New(events, "estoria.projection", newState); err != nil {
			t.Errorf("want the projection stream type allowed, got %v", err)
		}
	})
